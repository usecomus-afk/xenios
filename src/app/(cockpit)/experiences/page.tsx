    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          ...
          className="px-3 py-2.5 text-xs bg-[#171a22] border border-[#2c313d] rounded-xl text-zinc-300 max-w-xs"
        >
          {categories.map((c) => (
      ...
      {/* Table */}
      <div className="bg-[#171a22] rounded-3xl border border-[#2c313d] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-zinc-300">
            <thead className="bg-[#12141a] text-zinc-400 border-b border-[#2c313d] uppercase text-[10px]">
              <tr>
              ...
              {filtered.map((exp) => (
                <tr key={exp.id} className="hover:bg-white/5 transition">
                  <td className="p-4">
                    ...
                    <strong className="text-white text-xs">{exp.title}</strong>
                    <p className="text-[11px] text-zinc-500 line-clamp-1 mt-0.5">{exp.agentNote}</p>
                  </td>
                  <td className="p-4 font-semibold text-zinc-200">{exp.provider}</td>
                  <td className="p-4 text-zinc-400">{exp.location}</td>
                  <td className="p-4 font-mono text-[11px] text-zinc-300">{exp.phone}</td>
                  <td className="p-4 font-mono font-bold text-amber-400">
                    {formatPrice(exp.price, exp.currency)} <span className="text-[10px] font-normal text-zinc-400">({exp.duration})</span>
                  </td>
                  <td className="p-4">
