    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-400">Misafirlerin QR kod ile odalarından ilettiği tüm canlı istekler — modül bazlı iş akışı ve operasyon durumu</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-2xl bg-[#171a22] border border-[#2c313d]">
          ...
          <div className="text-xl font-bold text-white mt-1 font-mono">{stats.todayCount}</div>
        </div>
        <div className="p-3.5 rounded-2xl bg-[#171a22] border border-[#2c313d]">
      ...
      {stats.topDepartments.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 text-[11px]">
          ...
          {stats.topDepartments.map(([dept, count]) => (
            <span key={dept} className="px-2.5 py-1 rounded-lg bg-[#171a22] border border-[#2c313d] text-zinc-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex items-center gap-1.5 bg-[#171a22] p-1 rounded-xl border border-[#2c313d] overflow-x-auto">
        ...
        <select
          ...
          className="bg-[#171a22] border border-[#2c313d] text-zinc-300 text-xs rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-500/40"
        >
        ...
        <div className="space-y-3">
          {filtered.map((req) => {
            ...
            return (
              <div
                key={req.id}
                className="p-4 rounded-2xl bg-[#171a22] border border-[#2c313d] hover:border-amber-500/40 transition flex flex-col gap-3 text-xs overflow-hidden"
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div className="space-y-1.5 min-w-0 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {req.department && (
                        <span className="px-2 py-0.5 rounded-lg bg-[#0f1116] border border-[#2c313d] text-zinc-400 text-[10px] shrink-0">
                          {req.department}
                        </span>
                      )}
                    </div>
                ...
                {config && detailFields.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 bg-[#12141a] p-3 rounded-xl border border-[#2c313d]">
                    {detailFields.map((f) => {
                      ...
                      return (
                        <div key={f.key} className="flex items-baseline justify-between gap-2 text-[11px] border-b border-[#1d2028] pb-1 last:border-0">
                          ...
                          <span className="text-zinc-200 font-semibold text-right truncate">{value}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {config ? (
                    <>
                      <select
                        ...
                        className="bg-[#0f1116] border border-[#2c313d] text-zinc-200 text-[11px] font-semibold rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-amber-500/40"
                      >
                        {config.stages.map((s) => (
                      ...
                      {upcoming && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold transition flex items-center gap-1"
                        >
